...
CREATE POLICY "Users can view own agent_packages" ON public.agent_packages FOR SELECT TO authenticated USING (
    agent_id IN (SELECT id FROM public.agents WHERE sub_agency_id IN (SELECT id FROM public.sub_agencies WHERE agency_id IN (SELECT id FROM public.agencies WHERE user_id = auth.uid())))
);
CREATE POLICY "Users can insert own agent_packages" ON public.agent_packages FOR INSERT TO authenticated WITH CHECK (
    agent_id IN (SELECT id FROM public.agents WHERE sub_agency_id IN (SELECT id FROM public.sub_agencies WHERE agency_id IN (SELECT id FROM public.agencies WHERE user_id = auth.uid())))
);
CREATE POLICY "Users can update own agent_packages" ON public.agent_packages FOR UPDATE TO authenticated USING (
    agent_id IN (SELECT id FROM public.agents WHERE sub_agency_id IN (SELECT id FROM public.sub_agencies WHERE agency_id IN (SELECT id FROM public.agencies WHERE user_id = auth.uid())))
);
CREATE POLICY "Users can delete own agent_packages" ON public.agent_packages FOR DELETE TO authenticated USING (
    agent_id IN (SELECT id FROM public.agents WHERE sub_agency_id IN (SELECT id FROM public.sub_agencies WHERE agency_id IN (SELECT id FROM public.agencies WHERE user_id = auth.uid())))
);
